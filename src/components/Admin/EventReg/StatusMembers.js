...
const StatusMembers = () => {
  ...
  const { name,regno,status, date, time,users} = data;
  ...
  const getSinglePhoto = async () => {
    ...
    const snapshot = await getDoc(docRef, "EventRegistrations");
    if (snapshot.exists()) {
...
<Form.Input name="id" hidden
                  value={id}
                />


<Form.Input name="users" hidden
                  value={users}
                />
